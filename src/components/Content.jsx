...
const Content = () => {
  ...
  const bio = user.bio ? (
    <p className="mb-8 text-textDescr text-toggle dark:text-white">{user.bio}</p>
  ) : (
    <p className="mb-8 text-textDescr text-toggle dark:text-white">This profile has no bio</p>
  );

  const twitterLink = user.twitter_username ? (
    <a
      className="text-textDescr text-toggle hover:underline dark:text-white"
      href={`www.twitter.com/${user.twitter_username}`}
    >
      {user.twitter_username}
    </a>
  ) : (
    <p className="text-textDescr text-toggle dark:text-white">Not Available</p>
  );

  const blogLink = user.blog ? (
    <a className="text-textDescr text-toggle hover:underline dark:text-white" href={user.blog}>
      {user.blog}
    </a>
  ) : (
    <p className="text-textDescr text-toggle dark:text-white">Not Available</p>
  ...
  return (
    <div className="w-[730px] bg-windowBackground dark:bg-windowBackgroundDark rounded-windowRadius p-12">
      <div className="flex gap-[37px]">
        <div className="shrink-0">
          <img
            src={user.avatar_url}
            alt="logo"
            className="w-[117px] h-[117px] rounded-[117px] shrink-0"
          />
        </div>
        <div className="flex flex-col w-full items-start">
          <div className="w-full flex justify-between items-center mb-[2px]">
            <h1 className="text-headingM text-[#2B3442] dark:text-white">{user.name}</h1>
            <p className="text-textXss text-[#697C9A] dark:text-white dark:opacity-75">
      ...
      <div className="mt-[-30px]">
        <div className="ml-[154px]">
          {bio}
          <div>
            <div className="flex justify-between items-start text-left bg-background dark:bg-backgroundDark dark:text-white rounded-contentRadius px-8 py-4 mb-9">
              <div>
                <p className="text-textXss text-toggle mb-[1px] dark:text-white">Repos</p>
                <h3 className="text-textL text-[#2B3442] dark:text-white">{user.public_repos}</h3>
              </div>
              <div>
                <p className="text-textXss text-toggle mb-[1px] dark:text-white">Followers</p>
                <h3 className="text-textL text-[#2B3442] dark:text-white">{user.followers}</h3>
              </div>
              <div>
                <p className="text-textXss text-toggle mb-[1px] dark:text-white">Following</p>
                <h3 className="text-textL text-[#2B3442] dark:text-white">{user.following}</h3>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-y-5 text-toggle dark:text-white">
              <div className="flex gap-x-3">
                ...
                <p className="text-textDescr text-toggle dark:text-white">{user.location}</p>
              </div>
              <div className="flex gap-x-3 opacity-50">
              ...
              <div className="flex gap-x-3 text-toggle dark:text-white">
              ...
              <div className="flex gap-x-3 text-toggle dark:text-white">
                ...
                <p className="text-textDescr text-toggle dark:text-white">{user.company}</p>
              </div>
            </div>
          </div>
        </div>
